...
import (
	...
	"github.com/driif/echo-go-starter/pkg/cache"
	...
	return func(next echo.HandlerFunc) echo.HandlerFunc {
		return func(c echo.Context) error {
			...
			if len(cacheControl) > 0 {
				directive := cache.ParseCacheControlHeader(cacheControl)

				ctx := c.Request().Context()

				l := logs.LogFromContext(ctx).With().Str("cacheControl", directive.String()).Logger()
				ctx = l.WithContext(ctx)

				ctx = context.WithValue(ctx, logs.CTXKeyCacheControl, directive)
